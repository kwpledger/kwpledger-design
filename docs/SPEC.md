...
1. **The entry surface conforms.** A property's first screen — the title screen, the landing page, whatever a visitor sees before they have done anything — should read as belonging to the family. Interiors earn more latitude the further they get from that first impression, because by then the visitor is inside a specific thing rather than working out whose it is.
   ...
   **The header is the hard edge of this rule, and it is not negotiable** (Kevin, 2026-09-08). It is the first element of the first screen, so it is where "belonging to the family" is decided before a visitor reads a word. [header-footer-design-system.md](header-footer-design-system.md) is the rule; a surface conforms to it at the tier its platform allows, and the tiers are a strict subset chain rather than alternatives. Everything below in this section is about how much latitude an *interior* earns — none of it reaches the header.

   This was learned the expensive way. `runbox-mcp.kwpledger.com` was built by emulating a screenshot of kwpledger.com instead of consuming this system; every token value came out identical and the two headers still did not match, because the pages disagreed about what `<header>` means. Identical values are not conformance.
...
**The same test reaches into the format specs** (Kevin, 2026-09-23). A format document — the infographic system, the quote post, the banner — is guidance built on this one, and its rules bend on the same terms: the departure makes sense for what that particular piece is, and the piece records what it did and why that beat the alternative. The header rule above is the exception, as it is everywhere in this section. Each format spec names the constraints that stay fixed regardless; [infographic-design-system.md](infographic-design-system.md) Part 10 is the first to spell this out.
...
- ~~**Headers and footers.**~~ **Now specified — KWP-16 landed 2026-09-08.** See [header-footer-design-system.md](header-footer-design-system.md), and §10.1 above for why the header is the one place conformance is not negotiable. The old entry's reasoning still holds and is why that document names *parts* rather than shipping CSS: consumers vary from total control (kwpledger.com) to five theme settings (Substack) to fully generated (Base44) to a fixed canvas (the infographic format, which has its own footer spec), so a single rule cannot be about CSS. What changed is only that the rule now exists; "do not invent one" has been satisfied rather than lifted.

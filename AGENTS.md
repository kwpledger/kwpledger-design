...
fonts/             # woff2 + OFL-NOTICE.txt — the licence travels with the files
tools/
...
- **Colour is always reinforcement, never the sole carrier of meaning.** Every colour-coded axis carries a text label. Accessibility requirement, and it is what makes these values safe to change later.
...
- **Never exceed the chroma ceiling of 0.091** (`--teal-300`, the most saturated brand colour). Stately, not neon, expressed as a number a script can check.
- **Never let a value clip out of sRGB.** A clipped colour is silently no longer at its authored lightness, which breaks parity with no warning.
...
- **Do not build an authoring UI or tooling.** `tools/` holds a verifier and the maths it needs, and stays that size.
...
- **Never redraw the signature marks.** They are traced from Kevin's 2013 handwritten signature. Recolour if needed; do not vectorize, retrace, or regenerate the glyph.
- **Kevin's initials are lowercase — `kwp`, never `KWP`.** The signature monogram is the reason.
- Font licences travel with the font files. If you move `fonts/`, move `OFL-NOTICE.txt` with it.
